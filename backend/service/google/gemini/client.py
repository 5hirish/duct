...
One transport for both methods: client.models.generate_content, with any
reference images attached inline as parts. The Imagen branch that used to
sit alongside it (generate_images / edit_image with RawReferenceImage +
Mask/Style/Subject refs) is gone — the imagen-4.0-* models it addressed were
retired on 2026-08-17, and Google's replacement is gemini-3.1-flash-image,
which is a generate_content model.

  generate_image(request) — reference images ride ahead of the prompt.
  edit_image(request, base_bytes, ...) — edit-style continuation from the
                            base image inline. NOTE: mask/style/subject refs
                            have no generate_content equivalent yet; see the
...
class GeminiImageClient:
    ...
    async def generate_image(
        ...
        *,
        ...
        image_size rides in image_config; person_generation and
        negative_prompt are inert (they were Imagen options). Reference image
        bytes are attached inline as parts before the prompt.
    ...
    async def edit_image(
        ...
        *,
        ...
        # Masked / style / subject editing was an Imagen-only capability and
        # died with it — generate_content has no equivalent. The @tool layer
        # still accepts and loads these refs, so say so rather than dropping
        # them silently; restore a real path here if Google ships one.
        dropped = [
            n for n, b in (
        ...
        if dropped:
            logger.warning(
                "edit_image: ignoring %s reference(s) — unsupported by %s; "
                "the edit runs from the base image and prompt alone",
                "/".join(dropped), request.model.value,
            )
        return await asyncio.to_thread(
