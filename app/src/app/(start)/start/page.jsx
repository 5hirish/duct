...
import { AlertTriangle, ArrowRight, Check, ClipboardCheck, Globe, Sparkles } from "lucide-react";
...
export default function StartPage() {
    ...
    pollRef.current = setInterval(async () => {
      try {
        ...
        if (next.state !== "running") {
          ...
          // channels, a better name. Same merge, same rules.
  ...
  const readSite = useCallback(
    async (event) => {
      ...
      try {
        ...
        let status;
        try {
          status = await prefetchSite(target);
        } catch (err) {
        ...
        setCrawl(status);
        setProject(applyProjectDraft(status.draft));
  ...
  const startAudit = useCallback(
    ({ skipped = false } = {}) => {
      ...
      const params = {
        ...
        // Stripped before the request; the session page renders the "connect a
        // model" card first when it is set.
        ...(skipped ? { pending_provider: true } : {}),
      };
  ...
  const siteName = project?.company?.name || site?.title || site?.url || "";
  const pagesLine =
    crawl?.state === "running"
          ...
          {step === STEP.URL && siteFound && (
            <div className="grid gap-5">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <Button type="button" size="lg" onClick={() => setStep(STEP.PROVIDER)}>
