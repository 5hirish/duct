...
export default function ProjectAccountSelect({
  ...
  const foreignName = foreign
    ? binding.account_name || binding.account_id || t`a teammate's account`
    : "";
  ...
  return (
    ...
    <div className="conn-field">
      <Label htmlFor={`mapping-${projectName}`}>
        <Trans>Account</Trans>
      </Label>
      <Select
        ...
        <SelectContent>
          ...
          {rows.map((row) => (
            <SelectItem key={row.id} value={row.id}>
              {row.account_name || row.account_id || t`Unnamed account`}
            </SelectItem>
          ))}
        </SelectContent>
      </Select>
      <p className="conn-hint">
        {foreign ? (
          <Trans>Currently mapped to {foreignName} — a teammate's connector.</Trans>
        ) : (
          <Trans>
