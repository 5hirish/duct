...
import { connectedConnectorTypes } from "./connectorsApi";
...
export async function loadDesk({ projectId, activityLimit = 12, memoryLimit = 60 }) {
  if (!projectId) {
    return { memories: [], conversations: [], artifacts: [], activity: [], changeSets: [], sourceCount: 0 };
  }

  const [memory, conversations, artifacts, activity, changeSets, sources] = await Promise.all([
    ...
    orEmpty(connectedConnectorTypes(), new Set()),
  ]);
