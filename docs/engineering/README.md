...
- [`agent-memory-research.html`](agent-memory-research.html) — **research + design (Phase 1 built):** how ChatGPT, Claude, Claude Code, Hermes Agent and the memory frameworks (Mem0, Zep, Letta…) implement memory; the Duct model — user / project / artifact scopes in one bi-temporal `project_memories` table, agent `RememberFact`/`SearchMemory` tools, post-session consolidation, provenance chips and the project timeline. §07 carries the build status: what shipped in Phase 1 and what is still Phase 2+
