...
Ported near-verbatim from the Duct backend (backend/service/apify/client.py).
...
from .schema import ApifyRun, ScrapedPost
...
_USER_AGENT       = "TikTokResearchMCP/0.1 (+https://getduct.ai)"
...
class ApifyClient:
    ...
    async def start_run(self, actor_id: str, input_payload: dict[str, Any]) -> ApifyRun:
        ...
        # extra URL path segment, which Apify answers with a 404. Normalize the
        # slug to the tilde form.
    ...
    async def get_dataset_items(
        ...
        *,
        ...
        """Fetch raw items from a dataset. Returns list of dicts — the caller
        can model them via ScrapedPost when appropriate."""
    ...
    async def get_dataset_posts(
        ...
        *,
        ...
        """Like get_dataset_items but validates each item via ScrapedPost. Bad
        items are dropped (the actor evolves; one weird row shouldn't bork the
        whole flow) — but loudly, logged at WARNING with the field errors so the
        next shape drift is visible, not invisible."""
